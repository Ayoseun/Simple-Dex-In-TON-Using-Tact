...
function dictValueParserDeployedPair(): DictionaryValue<DeployedPair> {
 ...
 type TonkPumpDefaultWallet_init_args = {
    $$type: 'TonkPumpDefaultWallet_init_args';
    master: Address;
    owner: Address;
}

function initTonkPumpDefaultWallet_init_args(src: TonkPumpDefaultWallet_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.master);
        b_0.storeAddress(src.owner);
...
const TonkPumpDefaultWallet_types: ABIType[] = [
...
const TonkPumpDefaultWallet_getters: ABIGetter[] = [
...
const TonkPumpDefaultWallet_receivers: ABIReceiver[] = [
...
export class TonkPumpDefaultWallet implements Contract {
    
    static async init(master: Address, owner: Address) {
        return await TonkPumpDefaultWallet_init(master, owner);
    }
    
    static async fromInit(master: Address, owner: Address) {
        const init = await TonkPumpDefaultWallet_init(master, owner);
        const address = contractAddress(0, init);
        return new TonkPumpDefaultWallet(address, init);
    }
    
    static fromAddress(address: Address) {
        return new TonkPumpDefaultWallet(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  TonkPumpDefaultWallet_types,
        getters: TonkPumpDefaultWallet_getters,
        receivers: TonkPumpDefaultWallet_receivers,
        errors: TonkPumpDefaultWallet_errors,
    };
