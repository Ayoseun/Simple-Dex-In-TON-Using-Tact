...
describe('AMM Buy Token Test', () => {
    ...
    it('should validate that User can buy jetton', async () => {
        ...
        console.log("Initial balance Jetton Reserve:",fromNano( await tonkPumpAMM.getJettonTotalSupply()))
        expect(await tonkPumpAMM.getJettonTotalSupply()).toEqual(0n)
        await devMintTokens();
        expect(fromNano( await tonkPumpAMM.getJettonTotalSupply())).toEqual("15100000")
        console.log("With Dev Balance Jetton Reserve:",fromNano( await tonkPumpAMM.getJettonTotalSupply()))
    ...
    async function setupAMM() {
        const jettonAddress = await tonkPumpFactory.getJettonAddress(0n);
        const jettonAMMAddress = await tonkPumpFactory.getAmmContractAddress(0n);
